...
export default function LeaveTypeDrawer({ open, onClose, onSaved, companyId }) {
  ...
  async function handleSubmit(e) {
    ...
    const { error: dbError } = await supabase.from("leave_types").insert({
      company_id: companyId,
      name,
      default_days_per_year: Number(days || 0),
    });
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute top-0 right-0 bottom-0 w-full max-w-[380px] bg-white p-7 overflow-y-auto shadow-2xl animate-[slideIn_280ms_var(--ease-out)]">
        <h2 className="font-display text-lg font-semibold text-[var(--color-text-primary)]">Add leave type</h2>
        <p className="text-sm text-[var(--color-text-muted)] mt-1 mb-6">Available to every employee once added.</p>
