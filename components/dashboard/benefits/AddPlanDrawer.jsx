...
export default function AddPlanDrawer({ open, onClose, onSaved, companyId }) {
  ...
  async function handleSubmit(e) {
    ...
    const { error: dbError } = await supabase.from("benefit_plans").insert({
      company_id: companyId,
      name,
      category,
      provider: provider || null,
      description: description || null,
      employee_monthly_cost: Number(cost || 0),
    });
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute top-0 right-0 bottom-0 w-full max-w-[380px] bg-white p-7 overflow-y-auto shadow-2xl animate-[slideIn_280ms_var(--ease-out)]">
        ...
        <p className="text-sm text-[var(--color-text-muted)] mt-1 mb-6">
          Visible to everyone in the company once added.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--color-text-primary)] mb-1.5">
              Provider <span className="text-[var(--color-text-muted)] font-normal">(optional)</span>
            </label>
            <input
              type="text"
              value={provider}
